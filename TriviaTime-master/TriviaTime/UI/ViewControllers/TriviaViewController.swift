...
extension TriviaViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let ok = UIAlertAction(title: "Ok", style: .default, handler: { action in
            ...
            // In here, we have to create the triviaItem, then write and save to Realm
            let realm = try! Realm()
            let triviaItem = TriviaItem()
            triviaItem.answer = result.resolve().answer
            triviaItem.question = triviaController.question()
            triviaItem.correct = result.resolve().result
            try! realm.write {
                realm.add(triviaItem)
            }
            self.resetGame()
        })
